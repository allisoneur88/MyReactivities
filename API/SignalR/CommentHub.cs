...
public class CommentHub(IMediator mediator) : Hub
{
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        if (string.IsNullOrWhiteSpace(activityId)) throw new HubException("No activity with this id");

        await Groups.AddToGroupAsync(Context.ConnectionId, activityId!);

        var result = await mediator.Send(new GetComments.Querie { ActivityId = activityId! });
